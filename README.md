...
- **DataV 风格数据大屏**：深色科技风 + 6 张图表 + 实时刷新；**今日课堂行为**统一为**标准八类**展示（与视频分析归一一致）
- **演示模式（F9）**：按脚本自动巡游页面，支持**暂停/继续（空格）**、口播多行展示；长页面在单步停留内**自动滚动**便于看全内容
...
├── storage/              # 本地文件存储（自动创建）
├── docs/                 # 报告大纲、伦理、架构、M0~M10 完成清单
├── 算法.md               # 各模块指标与评分公式说明（与实现对齐）
└── README.md
```
...
| 班主任（七1班）| `head_7_1b` | `head1234` | 本班全科 + 自任教科目跨班 |
| 科任老师（物理）| `sub_physics` | `sub12345` | 自任教的「班级×科目」交叉范围 |

完整 16 个账号详见 [docs/M1-完成清单.md](docs/M1-完成清单.md)。

> 说明：**学校管理（新建/编辑/删除学校）仅超级管理员可用**；学校管理员账号默认只管理本校数据。
...
| **M9** | 增强功能（聚类/预测/图谱/多模态/干预闭环）打磨 | [文档](docs/M9-增强功能完成清单.md) |
| **M10** | 课堂视频识别深度改造：摄像头/课表/实时分析/调度器 | [文档](docs/M10-视频识别深度改造完成清单.md) |
| **收尾** | 八类行为归一、视频库/上传/删除、报告页回放与单帧检测、演示模式增强、`算法.md` | 见 `docs/版本迭代总览.md` §6 |
...
- [作品应用场景与设计理念](docs/作品应用场景与设计理念.md)
- [技术方案](docs/技术方案.md)
- [作品源代码说明](docs/作品源代码说明.md)
- [用户手册](docs/用户手册.md)
- [演示视频脚本](docs/演示视频脚本.md)
- [版本迭代总览（M0~M10 + 近期收尾）](docs/版本迭代总览.md)
- [算法与指标说明（算法.md）](算法.md)
- [架构设计](docs/架构设计.md)
- [伦理设计说明](docs/伦理设计说明.md)
- [CPU 降级部署说明](docs/CPU降级部署说明.md)
- [安装指南（文本）](docs/安装指南.txt)
- M0~M10 完成清单（按里程碑）
